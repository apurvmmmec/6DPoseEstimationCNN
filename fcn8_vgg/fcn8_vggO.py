...
class FCN8VGG:
    def __init__(self, batchSize, keepProb, num_classes=2, random_init_fc8=False,
              debug=False ,enableTensorboard=False, vgg16_npy_path=None):
        if vgg16_npy_path is None or not os.path.isfile(vgg16_npy_path):
            logging.error(("File '%s' not found. Provide a valid path."), vgg16_npy_path)
            sys.exit(1)
        ...
        self.random_init_fc8 =random_init_fc8
        self.debug = debug
    ...
    def inference(self, rgb):
        ...
        self.fc6 = tf.nn.dropout(self.fc6, keep_prob=self.keepProb)

        self.fc7 = self._fc_layer(self.fc6, "fc7")
        self.fc7 = tf.nn.dropout(self.fc7, keep_prob=self.keepProb)

        if self.random_init_fc8:
        ...
        self.upscore2 = self._upscore_layer(self.score_fr,
                                            ...
                                            num_classes=self.num_Classes,
                                            debug=self.debug, name='upscore2',
                                            ksize=4, stride=2)
        ...
        self.upscore4 = self._upscore_layer(self.fuse_pool4,
                                            ...
                                            num_classes=self.num_Classes,
                                            debug=self.debug, name='upscore4',
                                            ksize=4, stride=2)
        ...
        self.upscore32 = self._upscore_layer(self.fuse_pool3,
                                             shape=tf.shape(bgr),
                                             num_classes=self.num_Classes,
                                             debug=self.debug, name='upscore32',
                                             ksize=16, stride=8)
        ...
        self.upscore32_pred = self._score_layer(self.upscore32_1, "upscore32_pred",
                                                num_classes=self.num_Classes, in_features=self.num_Classes,
                                                stddev=0.001)

        self.logits = tf.reshape(self.upscore32_pred, (-1, self.num_Classes))
