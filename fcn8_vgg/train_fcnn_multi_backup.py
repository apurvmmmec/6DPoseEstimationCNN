...
tf.app.flags.DEFINE_string('logDir', './logs',
                           ...
                           """and checkpoint.""")
tf.app.flags.DEFINE_string('checkpointPath', './model',
                           """Directory where to write event logs """
                           """and checkpoint.""")
...
def testModel(args):
    ...
    vgg_fcn = fcn8.FCN8VGG(batchSize=1, keepProb=0.5, num_classes=2,
                            random_init_fc8=True, debug=(args.verbose > 0), enableTensorboard=args.tensorboard,
                            vgg16_npy_path=args.pretrained)

    vgg_fcn.inference(rgb=rgbImgBatch)
    ...
    probabilities = sess.run(vgg_fcn.probabilities)
    testDataGen.saveImage(probabilities);
