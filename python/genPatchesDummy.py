...
labelled_color_mpath = base_path + 'test/{:02d}/64bins/labelled_colored/{:04d}.png'
labelled_mpath = base_path + 'test/{:02d}/64bins/labelled/{:04d}.png'
...
def subSampleImages(n, w, h, cenR, cenC):
    ...
    for px in bgToUse:
        ...
        labelled_images.append('%03d%03d_%04d_%03d.png %s\n' % (r, c, n, 0, 0))
        lblCnt[num_classes-1, 0] = lblCnt[num_classes-1, 0] + 1
        ct += 1
...
trainScenes=[]
with open(trainScenes_file_path.format(objId), 'r') as f:
    print f.readlines
    trainScenes.append(f.readlines());

alternateSceneImages= (trainScenes[0][0::1000]);
# for imId in range(1, 2, 1):
for imId in alternateSceneImages:
    imId = int(imId)
    rgbImg = cv2.imread(rgb_in_mpath.format(objId, imId), -1)
    depImg = cv2.imread(depth_in_mpath.format(objId, imId), -1)
